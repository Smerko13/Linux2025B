...
struct guess_data {
    ...
    char guessed_key[MAX_GUESS_LENGTH];
    int client_id;
    bool has_guess;
};
...
// Global variables
struct guess_data current_guess;
pthread_cond_t guess_cond = PTHREAD_COND_INITIALIZER;
struct password_data pass_data;
...
{ 
    ...
    char gueesed_key [256];
    memset(gueesed_key, 0, sizeof(gueesed_key));
    ...
    while(true)
    {
        ...
        if (!current_guess.has_guess)
        {
            ...
            strncpy(current_guess.guessed_key, gueesed_key, key_len);
            current_guess.guessed_password[decrypted_length] = '\0';
            current_guess.guessed_key[key_len] = '\0';
...
void handle_guess(char* correct_password, int key_len, struct password_data* pass_data) {
    ...
    MTA_encrypt(current_guess.guessed_key, key_len, current_guess.guessed_password,
                strlen(current_guess.guessed_password), re_encrypted, &re_encrypted_len);

    if (re_encrypted_len == pass_data->encrypted_length &&
        memcmp(re_encrypted, pass_data->encrypted_password, re_encrypted_len) == 0) {
        is_encrypted = true;
